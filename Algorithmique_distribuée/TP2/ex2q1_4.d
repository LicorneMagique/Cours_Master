...
void spawnedFunc(int myId, int n) {
    ...
    send(ownerTid, CancelMessage(nbMsg));
}
...
void main() {
    ...
    writeln("Meilleur cas : ", min, " messages, ", double(min) / double(n) , " message/thread");
    writeln("Pire cas : ", max, " messages, ", double(max) / double(n), " message/thread");
    writeln(
        "Nombre moyen de messages envoyé : ", nbMsg / k, ", ",
