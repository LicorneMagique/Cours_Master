...
///////////////////////////////////////////////////////////
    ...
    // Allocation
    if ((presence = (bool*)malloc(G->n*sizeof(bool))) == NULL) { // presence
        report_error("main: malloc() error 3");
    }
    if ((degre_courant = (int*)malloc(G->n*sizeof(int))) == NULL) { // degre_courant
        report_error("main: malloc() error 3");
    }
    if ((stable = (bool*)malloc(G->n*sizeof(bool))) == NULL) { // meilleur stable trouvé
        report_error("main: malloc() error 3");
    }
    if ((stable_courant = (bool*)malloc(G->n*sizeof(bool))) == NULL) {
        report_error("main: malloc() error 3");
    }

    // Initialisation
    for (i = 0; i < G->n; i++) {
        // au début ils sont tous présents
        presence[i] = true;
