...
//-- template, penser a preciser les parametres d'instanciation.
 ...
 public:
  ...
  iterator begin() {return iterator(*this,0);} // Constructeur private
...
class Tableau_iterator
{
    ...
    T& operator*() const {return ptab->vect[pos];}
    T* operator->() const {return &(ptab->vect[pos]);}
    ...
    // le constructeur par copie et l'opération d'affectation sont géneres par le langage (et sont public)
    
    private :
    Tableau<T,agrandissement> * ptab;
    unsigned int pos;
    // constructeur prive
    Tableau_iterator(Tableau<T,agrandissement> & t, unsigned int p) : ptab(&t), pos(p) {}
